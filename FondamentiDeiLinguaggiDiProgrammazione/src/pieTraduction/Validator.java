...
import pieGrammar.PieGrammarParser.StatReturnContext;
import pieGrammar.PieGrammarParser.StructDefinitionContext;
import pieGrammar.PieGrammarParser.VardefContext;
 ...
 *
 */
public class Validator extends PieGrammarBaseListener {
	...
	public void enterStructDefinition(StructDefinitionContext ctx) {
		...
		if (sym != null) // struct and function names cannot overlap
			reportError(ctx.start, "symbol '" + structName
					+ "' already defined as '" + sym.toString() + "'");
		else {
			StructSymbol struct = new StructSymbol(ctx.start, structName,
					currentScope);
	...
	public void enterFunctionDefinition(FunctionDefinitionContext ctx) {
		...
		if (sym != null) // struct and function names cannot overlap
			reportError(ctx.start, "symbol '" + functName
					+ " already defined as '" + sym.toString());
		else {
			FunctionSymbol fun = new FunctionSymbol(ctx.start, functName,
					currentScope, ctx.block());
	...
	public void exitVardef(VardefContext ctx) {
		...
		if (currentScope instanceof FunctionSymbol) {
			...
			if (fun.checkIfDefinedAsFormalParameter(varName))
				reportError(ctx.start, "duplicate argument '" + varName
						+ "' in function '" + fun.getName() + "' definition");
			VariableSymbol var = new VariableSymbol(ctx.start, varName,
					currentScope);
		...
		if (currentScope instanceof StructSymbol) {
			...
			if (struct.checkIfDefinedAsField(varName))
				reportError(ctx.start, "duplicate field '" + varName
						+ "' in struct '" + struct.getName() + "' definition");
			VariableSymbol var = new VariableSymbol(ctx.start, varName,
					currentScope);
			currentScope.define(var);
		}
